...
describe("GET /companies", function () {
  ...
  test("fails minEmp > maxEmp", async function () {
    ...
    expect(resp.text).toEqual(
      '{"error":{"message":"minEmployees cannot be greater than maxEmployees","status":400}}'
    );
  });
  ...
  test("fails inappropriate filter criteria", async function () {
    ...
    expect(resp.statusCode).toEqual(400);
    expect(resp.text).toEqual(
      '{"error":{"message":["instance is not allowed to have the additional property \\"logo\\""],"status":400}}'
    );
  });
